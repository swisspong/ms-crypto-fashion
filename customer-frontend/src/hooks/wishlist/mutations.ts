import { deleteManyItemInWishlist, postWishList } from "@/src/services/wishlist.service";
...
export const useRemoveManyItemInWishlist = () => {
    const queryClient = useQueryClient();
    return useMutation(
        (data: IDeleteManyItem) => deleteManyItemInWishlist(data),
        {
            // When mutate is called:
            onMutate: async (info) => {
                // Cancel any outgoing refetches (so they don't overwrite our optimistic update)
