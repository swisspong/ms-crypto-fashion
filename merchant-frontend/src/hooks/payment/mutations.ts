import { postCreditCard, postOmiseToken, postWithdraw, postWithdrawEth } from "@/src/services/payment.service";
...
export const useWithdrawEth = () => {
    const queryClient = useQueryClient();
    return useMutation((body: IWithdrawEthPayload) => postWithdrawEth(body), {
        // When mutate is called:
        onMutate: async (info) => {
            // Cancel any outgoing refetches (so they don't overwrite our optimistic update)
            await queryClient.cancelQueries(["payment"]);
